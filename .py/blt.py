...
def scroll_and_capture(page, row_sel: str, scroll_target_sel: str, max_seconds: int, limit: int = 0) -> List[List[str]]:
    ...
    while True:
        ...
        if new_pos <= prev:
            ...
            # Smart Threshold:
            # If (total is known) AND (current < total):
            #    If we are physically at the end (last_pos >= total), use lower threshold (20) to avoid infinite loops on ghost rows.
            #    Else (stuck in middle), use high threshold (200) to persist through heavy lags.
            # Else (unknown total OR reached goal), use low threshold (25).
            if total and current_len < total:
                threshold = 20 if last_pos >= total else 200
            else:
                threshold = 25
