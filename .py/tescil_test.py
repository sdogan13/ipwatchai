# tescil_test.py
# Search by Tescil Yayin Bulten No (Registration Bulletin Number), load full rows, and extract data to JSON.
...
# UPDATED: Gracefully handles Ctrl+C to save partial progress before exiting.
...
def locate_bulten_no_input(page):
    """
    Locates the input field for 'Tescil Yayın Bülten No'.
    Includes logic to expand 'Detaylı Arama' if the field is not initially visible.
    Uses Robust Regex Matching to handle case/encoding issues (u vs ü).
    """
    # Regex to match "Tescil ... B(u/ü)lten" case-insensitively
    tescil_pattern = re.compile(r"Tescil.*B[uü]lten", re.I)
    # Regex to match "Marka ... B(u/ü)lten" case-insensitively (for relative positioning)
    marka_pattern = re.compile(r"Marka.*B[uü]lten", re.I)

    def _find_target_input():
        # Strategy 1: Direct Label Match
        # Find mat-form-field containing the specific regex text
        target = page.locator("mat-form-field").filter(has_text=tescil_pattern).locator("input").first
        if target.count() > 0 and target.is_visible():
            log("[INFO] Found 'Tescil' input via direct label match.")
            return target
        return None

    # 1. Try to find the SPECIFIC 'Tescil' input immediately
    target_input = _find_target_input()
    if target_input: return target_input

    # 2. If specific input NOT visible, try clicking "Detaylı Arama"
    log("[INFO] Specific 'Tescil' input not visible. Expanding 'Detaylı Arama'...")
    try:
        # Try multiple selectors for the expansion button
        btn = page.locator("text='Detaylı Arama'").first
    ...
    # 3. Try Strategy 1 again after expansion
    target_input = _find_target_input()
    if target_input: return target_input

    # 4. Strategy 2: Relative Position (The box BELOW 'Marka')
    log("[INFO] Label match failed. Trying relative positioning (box below Marka)...")
    marka_field = page.locator("mat-form-field").filter(has_text=marka_pattern).first
    
    if marka_field.count() > 0:
        log("[INFO] Found 'Marka' anchor. Targeting the input immediately following it...")
        # Get the input of the Marka field, then the next input in DOM
        # XPath logic: (MarkaInput)/following::input[1]
        relative_input = marka_field.locator("input").locator("xpath=following::input[1]")
        if relative_input.count() > 0:
            return relative_input

    # 5. Strategy 3: Blind Indexing (Select the 2nd Bulletin Input)
    log("[WARN] Relative positioning failed. Attempting index-based selection...")
    # Find all inputs that look like Bulletin Number inputs
    generic_inputs = page.locator("input").filter(has=page.locator("xpath=ancestor::mat-form-field[contains(., 'Bülten') or contains(., 'Bulten') or contains(., 'No')]"))
    
    # If filter fails, try broader placeholder match
    if generic_inputs.count() == 0:
        generic_inputs = page.locator("input[placeholder*='Bülten'], input[placeholder*='Bulten'], input[aria-label*='Bülten']")

    count = generic_inputs.count()
    if count >= 2:
        log(f"[INFO] Found {count} potential inputs. Selecting the 2nd one (assuming it is Tescil).")
        return generic_inputs.nth(1)
    
    if count == 1:
        log("[WARN] Only 1 generic input found. This is likely the 'Marka' box, but returning it as a last resort.")
        return generic_inputs.first

    log("[ERROR] Could not locate 'Tescil Yayın Bülten No' input field.")
    return None
...
# CHANGED: Now returns a Tuple(Data, InterruptedBool)
def scroll_and_capture(page, row_sel: str, scroll_target_sel: str, max_seconds: int, limit: int = 0) -> Tuple[List[List[str]], bool]:
    ...
    try:
        while True:
            ...
            if new_pos <= prev:
                ...
                # FIXED THRESHOLD: As requested, set max stagnation to 30 for all cases.
                threshold = 30
        ...
        log("\n[WARN] User interrupted (Ctrl+C). Stopping scroll to save data...")
        interrupted = True

    return list(captured_data.values()), interrupted
...
def process_brand(context, page, bulten_no: str, out_dir: Path, max_scroll_seconds: int, limit: int) -> bool:
    ...
    all_rows, interrupted = scroll_and_capture(page, row_sel, scroll_target_sel, max_scroll_seconds, limit)
    
    # Ensure the target directory exists
    safe_mkdir(out_dir)
    # CHANGED: Save as metadata.json instead of {number}.json
    json_path = out_dir / "metadata.json"        # <-- Hardcoded to metadata.json
...
def main():
    ...
    # LOGIC ADDED: Expand ranges in names (e.g., "435-499" -> "435", "436", ...)
    final_names = []
    for val in args.names:
        # Check if input matches "number-number" pattern
        if '-' in val and re.match(r'^\d+-\d+$', val):
            try:
                start, end = map(int, val.split('-'))
                if start <= end:
                    # Add all numbers in the range to the list
                    final_names.extend(str(i) for i in range(start, end + 1))
                else:
                    final_names.append(val) # Keep as is if start > end
            except ValueError:
                final_names.append(val)
        else:
            final_names.append(val)
    
    log(f"[INFO] Processing {len(final_names)} bulletin numbers: {final_names[0]} ... {final_names[-1]}")

    # LOGIC CHANGED: Find the first missing GZ_N folder starting from 1 to determine start_seq
    # This prevents jumping to GZ_500 if GZ_499 exists but GZ_24 is the next logical sequence.
    start_seq = 1
    if ROOT_DIR.exists():
        while (ROOT_DIR / f"GZ_{start_seq}").exists():
            start_seq += 1
    
    log(f"[INFO] First available folder slot is GZ_{start_seq}. New batch will start there.")

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=args.headless)
        # CHANGED: Use start_seq to continue numbering from where we left off
        for idx, val in enumerate(final_names, start=start_seq):
            dynamic_out_dir = ROOT_DIR / f"GZ_{idx}"
            ...
            try: 
                ...
                stop_signal = process_brand(context, page, val, dynamic_out_dir, args.max_scroll_seconds, args.limit)
                
                if stop_signal:
                    log("[WARN] Script stopped by user request. Exiting batch processing.")
                    break
            except Exception as e: 
