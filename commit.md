...
- Created `codex/reduce-test-account-churn` from the merged mainline to isolate the disposable-account cleanup work instead of editing directly on `main`.
- Added deterministic managed free/starter/professional smoke personas plus a dedicated purge utility, and rewired the live/browser/nightly persona helpers so routine aggregate runs reuse those managed accounts instead of self-registering new random users.
- Verification exposed one real state-isolation regression after the reuse change: `tests/live/features/test_reports_live.py` exhausted the shared free account's monthly report quota because it never cleaned up generated reports. Fixed that by expanding the report cleanup helper to support whole-organization cleanup and resetting the managed reports suite before and after execution.
- Verified the account-churn fix with `python -m py_compile` on the updated helpers, `python tests/test_live_app_e2e.py`, `python tests/test_browser_e2e.py`, and `python tests/test_nightly_e2e.py`; the disposable-account audit stayed flat at `1068` before and after every aggregate lane, confirming the new harness stopped adding backlog users.
- Purged the accumulated legacy disposable backlog with `python scripts/devtools/purge_test_accounts.py --apply`, deleting `1068` disposable organizations/accounts plus the remaining dependent payment rows/files, then re-audited to confirm `0` disposable test accounts remained and reran `python tests/test_live_app_e2e.py` successfully against the cleaned database.
