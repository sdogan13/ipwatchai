...
- `tests/browser/test_trademark_search_filters_browser.py`: trademark Nice class picker — multi-select chips of class numbers 1..45 (UNIQUE — cografi has no class concept, patent uses IPC, design uses Locarno). Skips AI-Suggest path; exercises Browse-All-45 path via Alpine `toggleBrowseClass(num)` mutation. Discovery: `#dashboard-quick-search-btn` is `:disabled` while `searchLoading=true`, so follow-up submits invoke `dashboardQuickSearch()` directly via Alpine to bypass the disabled state. Asserts GET `/api/v1/search?query=Coca&classes=9` then resubmit with class 35 added → `classes=9,35`. Uses managed-professional persona.
