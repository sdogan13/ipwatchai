...
- the Docker-backed live/browser/nightly lanes are currently validated with `WORKERS=1`; the previous four-worker default caused intermittent empty-response failures on `/api/v1/search` and `/api/v1/search`
...
- `tests/browser/test_design_search_filters_browser.py`: design search filter panel — Locarno class finder via "Browse All 32 Classes" path; click class 06 from the browse list, assert hidden #design-search-locarno populates, assert the selected-class chip renders, submit search and verify the locarno param appears in the POST multipart body. The AI-suggest path is NOT exercised (costs AI credits the persona may lack). Uses managed-professional persona.
- `tests/browser/test_design_admin_actions_browser.py`: design watchlist admin toolbar — API pre-create 2 design watches, then exercise three design-UNIQUE admin actions: (1) **Scan All** which hits a single POST /api/v1/design-watchlist/scan-all (different from patent/cografi which fire N parallel per-item /scan calls from the browser); (2) **Threshold slider** value change is accepted (the bulk-threshold PUT may not fire on every change — design's threshold may filter client-side); (3) **Delete All** which hits DELETE /api/v1/design-watchlist/all and clears the list. Uses managed-starter persona.
- `tests/browser/test_design_image_search_browser.py`: design image-upload search (DINOv2 + CLIP + HSV histogram triple-signal scoring; design is the most image-first registry) — API search to find a design with image_url, download the JPEG, attach to #design-search-image via set_input_files, fire hybrid text+image search via Lamba query, assert POST 200 + non-empty results + source record self-matches in the rendered grid. Uses managed-professional persona.
- `tests/browser/test_design_inline_filter_sort_browser.py`: design inline watchlist filter + sort — both controls (`#dwl-search-input` with 200ms debounce + `#dwl-sort-select`) re-render `#design-watchlist-list > article` rows CLIENT-SIDE with no HTTP traffic. API pre-create 3 smoke rows (alpha/beta/gamma); type 'alpha' into the filter input + assert exactly 1 smoke row remains visible + assert ZERO requests fired to `/design-watchlist` or `/design-alerts` during filtering; click clear-X + assert all 3 return; switch sort to `name_asc` + assert smoke rows render in alphabetical order. Uses managed-starter persona.
- `tests/browser/test_design_edit_modal_browser.py`: design watchlist edit modal (`#design-watchlist-edit-modal`) — UNIQUE to design; patent + cografi have no equivalent edit UX. API pre-create 1 watch, click per-row `button[data-action="edit"]`, assert modal visible + inputs prefilled (product_name + locarno + threshold), change name + threshold + flip visual monitoring checkbox, click `#edit-dwl-submit-btn` + assert PUT `/api/v1/design-watchlist/{id}` returns 200, modal auto-closes (.hidden class re-applied), loadList() reruns + row reflects EDITED_NAME (ORIGINAL_NAME no longer in row text). Uses managed-starter persona.
- `tests/browser/test_design_bulk_upload_browser.py`: design watchlist bulk CSV upload (`#design-watchlist-upload-modal`) — UNIQUE to design; biggest novel surface in the design UI. Three-step modal flow: (1) write 2-row CSV to temp file + click `#dwl-btn-bulk-upload` + attach via `#dwl-upload-file` + click Detect Columns → POST `/upload/detect-columns` 200; (2) step-2 renders mapping selects with `data-dwl-map-field` attrs + `product_name` column auto-mapped from suggested_mapping; (3) submit → POST `/upload/with-mapping` 200 with `added >= 1` + result body rendered + auto-reloaded list shows both new rows. Uses managed-starter persona.
- `tests/browser/test_design_locales_browser.py`: design subview locale rendering (TR/EN/AR) via `window.AppI18n.setLocale()` — switches programmatically (locale-agnostic vs depending on header dropdown markup), verifies design watchlist subview's two probe labels render in target locale: `design_watchlist.filter_sort` (Sort/Sırala/الترتيب) + `design_watchlist.stat_total_items` (Total/Toplam/الإجمالي), and for AR also asserts `html[dir]="rtl"` + `html.rtl` class is set; for EN asserts LTR. Uses managed-starter persona.
- `tests/browser/test_trademark_admin_actions_browser.py`: trademark watchlist admin toolbar — three actions with discoveries vs the other 3 registries: (1) **Scan All** opens a CUSTOM in-DOM confirmation overlay (`#scan-confirm-overlay` + `#scan-confirm-ok`), NOT a native confirm() like design/cografi/patent; OK click fires single POST `/scan-all`. (2) **Delete All** fires TWO native confirm() dialogs (double confirmation, UNIQUE — the other 3 fire one). (3) **Threshold slider** (#wl-threshold-slider, a `<select>`) is a CLIENT-SIDE display filter — assertion: no PUT to `/bulk-threshold` or `/threshold` endpoint. Uses managed-starter persona.
- `tests/browser/test_trademark_edit_modal_browser.py`: trademark `#watchlist-edit-modal` — UNIQUE input vs design is `#edit-wl-monitor-phonetic` (phonetic monitoring, trademark-only concept). Discovery: `openEditWatchlistModal(idx)` takes the INDEX into `window._watchlistItemsCache`, NOT an item id — slice looks up the cache index of its smoke row before invoking. Flow: pre-create with phonetic=True; open via Alpine; assert prefilled including phonetic CHECKED; change brand+threshold+UNCHECK phonetic; submit + assert PUT 200; cache+DOM reflect EDITED_NAME after auto-reload. Uses managed-starter persona.
- `tests/browser/test_trademark_inline_filter_sort_browser.py`: trademark watchlist filter+sort is SERVER-SIDE (opposite of design's client-side). Every change to `#wl-search-input`, `#wl-sort-select`, or `#wl-tm-status-select` fires GET `/api/v1/watchlist?<params>` via `loadPortfolio()`. Three assertions parse the URL with `urllib.parse.parse_qs`: search input → carries `search=<term>`; sort select → carries `sort=name_asc`; **`#wl-tm-status-select`** (UNIQUE — the other 3 registries have no tm_status concept) → carries `tm_status=<value>`. Uses managed-starter persona.
- `tests/browser/test_trademark_search_filters_browser.py`: trademark Nice class picker — multi-select chips of class numbers 1..45 (UNIQUE — cografi has no class concept, patent uses IPC, design uses Locarno). Skips AI-Suggest path; exercises Browse-All-45 path via Alpine `toggleBrowseClass(num)` mutation. Discovery: `#dashboard-live-search-btn` is `:disabled` while `searchLoading=true`, so follow-up submits invoke `dashboardAgenticSearch()` directly via Alpine to bypass the disabled state. Asserts GET `/api/v1/search?query=Coca&classes=9` then resubmit with class 35 added → `classes=9,35`. Uses managed-professional persona.
- `tests/browser/test_trademark_result_expansion_browser.py`: trademark uses INLINE result expansion — clicking a result-card header toggles `expandedResult = i` on the Alpine search-state scope, revealing an x-collapse panel. Structurally different from cografi/patent/design which use dedicated detail modals. The score breakdown panel renders 3 columns: text + visual + **phonetic** (last column UNIQUE to trademark). Slice asserts: expandedResult is null initially, set=0 reveals breakdown, all 3 column labels resolve via `t()` and appear in DOM, reset=null collapses. Uses managed-professional persona.
- `tests/browser/test_trademark_image_search_browser.py`: trademark image-search comprehensive — adds depth to existing `test_search_browser.paid_image_search` (which uses synthetic 1×1 PNG). Downloads a REAL image from the live trademark corpus (text-search "Coca" → first result with image_url → fetch bytes), submits POST `/api/v1/search` (multipart) with hybrid text+image, asserts source record SELF-MATCHES in rendered results — real signal that DINOv2 + CLIP scoring is functional end-to-end. No Nice class filter (combining with class can exclude the source if it sits in a different class). Uses managed-professional persona.
- `tests/browser/test_trademark_bulk_upload_browser.py`: trademark bulk upload happy path — orthogonal to existing `test_member_feature_browser.watchlist_bulk_upload_limit_gate` which only covers the FREE-tier 403 plan-cap response. This slice is the PAID happy path: full 3-step flow lands rows in the watchlist. Step 1: file pick (`#upload-wl-file` accepts .xlsx/.xls/.csv) + `#upload-wl-detect-btn` → POST `/upload/detect-columns`. Step 2: mapping selects use ids `#upload-map-<field>` (NOT `data-*-map-field=` like design); `#upload-map-brand_name` auto-mapped + click `#upload-wl-submit-btn` → POST `/upload/with-mapping`. Step 3: result body in `#upload-wl-result` + auto-reload puts both rows in `_watchlistItemsCache`. Uses managed-starter persona.
- `tests/browser/test_trademark_locales_browser.py`: trademark watchlist subview locale rendering (TR/EN/AR) via `window.AppI18n.setLocale()`. Probe labels: `watchlist.filter_sort` (Sort/Sırala/ترتيب) + `watchlist.stat_total_items` (Total Items/Toplam Öğe/إجمالي العناصر). Trademark uses the BARE `watchlist.X` namespace (no registry prefix — predates the cografi/patent/design suffixed namespaces). For AR also asserts `html[dir]="rtl"` + `html.rtl` class set; for EN asserts LTR. Uses managed-starter persona.
...
- `tests/browser/test_patent_watch_types_browser.py`: patent holder watch_type round-trip — covers the watch_type that slice 1 (dashboard) doesn't (slice 1 round-trips reference). Create holder watch via UI → scan returns 200 → delete via UI with confirm() auto-accept. Uses managed-starter persona.
- `tests/browser/test_patent_alert_actions_browser.py`: patent alert action lifecycle — POST verb-endpoint flow (acknowledge/resolve/dismiss; different from cografi's PATCH-with-status-body shape). Creates an Arçelik holder watch, scans (live corpus produces alerts), then for each verb clicks the corresponding button on a distinct alert + asserts POST returns 200 + a follow-up GET shows the persisted status. Uses managed-starter persona.
- `tests/browser/test_patent_search_filters_browser.py`: patent search filter panel — exercises every filter dimension via FormData-body inspection. **IPC autocomplete** (typeahead → dropdown → chip → POST body includes the picked code; UNIQUE patent UX); holder text filter; kind code select; date range; combined query+holder+date. Uses managed-professional persona (search-heavy slice, 2000/day search quota vs starter's 50/day).
- `tests/browser/test_patent_admin_actions_browser.py`: patent watchlist scan-all toolbar action — API pre-create 2 holder watches → click Scan All → auto-accept confirm() dialog → capture all parallel POST /scan responses via page.on('response') → assert at least 2 fired + all 200 → after refresh, verify both rows show populated 'Last scan' (NOT the localized 'never' fallback). Patent has no edit-modal UI (only API-level PATCH); scan-all is the only admin action with a UI surface. Uses managed-starter persona.
- `tests/browser/test_patent_detail_modal_browser.py`: patent detail modal sub-section render — searches a stable common term (`kompozisyon`), opens the first result, asserts every section hydrates with real data (header title + record_type badge + kind code + publication_no, dates row, abstract NOT the no-abstract fallback, IPC chips ≥1, holders ≥1, source provenance footer non-placeholder). Uses managed-professional persona.
- `tests/browser/test_patent_image_search_browser.py`: patent image-upload search (DINOv2 figure similarity, hybrid text+image mode) — finds a record carrying image_url via API search, downloads its primary figure JPEG, attaches via page.set_input_files to #patent-search-image, fires hybrid search with text query "kompozisyon", verifies POST 200 + source record self-matches in rendered grid. Uses managed-professional persona.
- `tests/browser/test_patent_locales_browser.py`: patent UI EN/TR/AR locale comprehensive — for each locale verifies search input placeholder text, IPC label (UNIQUE patent UX), holder filter label, watch_type=holder radio label, and html dir attribute (ltr for EN/TR, rtl for AR). setLocale may navigate the page on RTL switch (handled with try/except + wait_for_load_state). Uses managed-starter persona.
- `tests/browser/test_patent_leads_browser.py`: patent Leads dashboard surface (UNIQUE — no cografi equivalent; lives at radarView==='patent' inside the radar tab) — verifies all 4 category buttons rendered (lapse/transfer/license/rejected), category switch GET fires with category param, holder filter sends URL-encoded holder param, watchlist-scope toggle sends watchlist_scoped=true, CSV export download triggers + non-empty CSV. Uses managed-professional persona (patent-leads endpoint is plan-gated; starter returns 403 upgrade_required). Note: the category-button click via Playwright doesn't reliably trigger the JS document-level click delegation in patent_leads.js wire(); the test workaround invokes the load URL directly via fetch() to verify the loader builds the URL correctly. The click→loader wire is real coverage gap worth a separate look.
...
- Added `tests/browser/test_live_search_browser.py` so browser coverage now proves the free-plan live-search upgrade gate and a real business live-search success path against `/api/v1/search`, including credit consumption and live-source results.
