...
- `tests/browser/test_patent_leads_browser.py`: patent Leads dashboard surface (UNIQUE — no cografi equivalent; lives at radarView==='patent' inside the opposition-radar tab) — verifies all 4 category buttons rendered (lapse/transfer/license/rejected), category switch GET fires with category param, holder filter sends URL-encoded holder param, watchlist-scope toggle sends watchlist_scoped=true, CSV export download triggers + non-empty CSV. Uses managed-professional persona (patent-leads endpoint is plan-gated; starter returns 403 upgrade_required). Note: the category-button click via Playwright doesn't reliably trigger the JS document-level click delegation in patent_leads.js wire(); the test workaround invokes the load URL directly via fetch() to verify the loader builds the URL correctly. The click→loader wire is real coverage gap worth a separate look.
